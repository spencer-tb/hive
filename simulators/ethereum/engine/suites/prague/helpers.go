...
func GetBlobGasPrice(excessBlobGas uint64) uint64 {
	return FakeExponential(prague.MIN_DATA_GASPRICE, excessBlobGas, prague.BLOB_GASPRICE_UPDATE_FRACTION)
}
...
func GetMinExcessBlobGasForBlobGasPrice(data_gas_price uint64) uint64 {
	...
	for current_data_gas_price < data_gas_price {
		current_excess_data_gas += prague.GAS_PER_BLOB
...
func GetMinExcessBlobsForBlobGasPrice(data_gas_price uint64) uint64 {
	return GetMinExcessBlobGasForBlobGasPrice(data_gas_price) / prague.GAS_PER_BLOB
}

func CalcExcessBlobGas(parentExcessBlobGas, parentBlobGasUsed uint64) uint64 {
	if (parentExcessBlobGas + parentBlobGasUsed) < prague.TARGET_BLOB_GAS_PER_BLOCK {
		return 0
	} else {
		return (parentExcessBlobGas + parentBlobGasUsed) - prague.TARGET_BLOB_GAS_PER_BLOCK
	}
}
...
func BeaconRootStorageIndexes(timestamp uint64) (common.Hash, common.Hash) {
	// Calculate keys
	timestampReduced := timestamp % prague.HISTORY_BUFFER_LENGTH
	timestampExtended := timestampReduced + prague.HISTORY_BUFFER_LENGTH
