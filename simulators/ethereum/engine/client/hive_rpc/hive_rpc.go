...
func (ec *HiveRPCEngineClient) NewPayload(ctx context.Context, version int, payload interface{}, versionedHashes *[]common.Hash, parentBeaconRoot *common.Hash) (result api.PayloadStatusV1, err error) {
	if err := ec.PrepareDefaultAuthCallToken(); err != nil {
		return result, err
	}

	if version >= 3 {
		err = ec.c.CallContext(ctx, &result, fmt.Sprintf("engine_newPayloadV%d", version), payload, versionedHashes, parentBeaconRoot)
	} else {
...
func (ec *HiveRPCEngineClient) NewPayloadV3(ctx context.Context, payload *typ.ExecutableData, versionedHashes *[]common.Hash, parentBeaconRoot *common.Hash) (api.PayloadStatusV1, error) {
	ec.latestPayloadSent = payload
	return ec.NewPayload(ctx, 3, payload, versionedHashes, parentBeaconRoot)
}
