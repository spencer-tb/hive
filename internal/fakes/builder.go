...
type BuilderHooks struct {
	...
	BuildSimulatorImage func(context.Context, string, map[string]string, map[string]string) (string, error)
...
func (b *fakeBuilder) BuildSimulatorImage(ctx context.Context, sim string, buildArgs, buildSecrets map[string]string) (string, error) {
	if b.hooks.BuildSimulatorImage != nil {
		return b.hooks.BuildSimulatorImage(ctx, sim, buildArgs, buildSecrets)
	}
